...
function columnsPreprocessor(array, schema) {
    ...
    for (const field of fields) {
        let datatype = null;
        let format = null;
        if (schema?.properties[field]?.pattern) {
            const datatypeAndFormat = getDatatypeFromRegex(schema?.properties[field]?.pattern)
            datatype = datatypeAndFormat.datatype
            format = datatypeAndFormat.format
        } else if (schema?.properties[field]?.format) {
            datatype = schema?.properties[field]?.format
        }
        arrayValuesObj[field] = { datatype: datatype, format: format, values: [] }
    }
...
function autofix(array, schema) {
    try {
        const processedColumnsObj = columnsPreprocessor(array, schema);
